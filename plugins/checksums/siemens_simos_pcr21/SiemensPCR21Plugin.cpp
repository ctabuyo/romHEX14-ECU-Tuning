...
#include "SiemensPCR21Plugin.h"

bool SiemensPCR21Plugin::canHandle(const QByteArray& rom, const QString& ecuType) const {
    Q_UNUSED(rom);
    const QString q = ecuType.toUpper();
    return (q.contains("PCR2.1") || q.contains("SIMOS PCR2.1") || q.contains("TC1796") || q.contains("TC1766"));
}

int SiemensPCR21Plugin::verify(const QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
...
int SiemensPCR21Plugin::correct(QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
