...
#include "TRWLucasPlugin.h"

bool TRWLucasPlugin::canHandle(const QByteArray& rom, const QString& ecuType) const {
    Q_UNUSED(rom);
    const QString q = ecuType.toUpper();
    return (q.contains("DCN2") || q.contains("LUCAS") || q.contains("TRW") || q.contains("EPIC"));
}

int TRWLucasPlugin::verify(const QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
...
int TRWLucasPlugin::correct(QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
