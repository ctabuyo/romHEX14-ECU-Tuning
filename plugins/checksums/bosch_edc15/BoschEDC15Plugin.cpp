...
#include "BoschEDC15Plugin.h"

bool BoschEDC15Plugin::canHandle(const QByteArray& rom, const QString& ecuType) const {
    Q_UNUSED(rom);
    const QString q = ecuType.toUpper();
    return (q.contains("EDC15") || q.contains("EDC15P") || q.contains("EDC15V") || q.contains("EDC15VM") || q.contains("29F400"));
}

int BoschEDC15Plugin::verify(const QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
...
int BoschEDC15Plugin::correct(QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
