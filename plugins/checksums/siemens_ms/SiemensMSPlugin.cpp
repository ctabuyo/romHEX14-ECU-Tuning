...
#include "SiemensMSPlugin.h"

bool SiemensMSPlugin::canHandle(const QByteArray& rom, const QString& ecuType) const {
    Q_UNUSED(rom);
    const QString q = ecuType.toUpper();
    return (q.contains("MS42") || q.contains("MS43") || q.contains("MS45") || q.contains("MSS54") || q.contains("MSS54HP"));
}

int SiemensMSPlugin::verify(const QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
...
int SiemensMSPlugin::correct(QByteArray& rom, QString& errorMsg) {
    Q_UNUSED(rom);
